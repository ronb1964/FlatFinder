...
export function describeAttitude(
  ...
  const pitchDescription = Math.abs(attitude.pitch) < threshold 
    ? 'Level' 
    : attitude.pitch > 0 
      ? `${attitude.pitch.toFixed(1)}° nose up`
      : `${Math.abs(attitude.pitch).toFixed(1)}° nose down`;
      
  const rollDescription = Math.abs(attitude.roll) < threshold
    ? 'Level'
    : attitude.roll > 0
      ? `${attitude.roll.toFixed(1)}° right high`
      : `${Math.abs(attitude.roll).toFixed(1)}° left high`;
      
  const overallDescription = isLevel 
    ? 'Level' 
    : `${pitchDescription}, ${rollDescription}`;
...
export const SENSOR_NORMALIZATION_PRESETS = {
  ...
  EXPO_DEVICE_MOTION: {
    ...
    invertRoll: false,   // rotation.gamma: +right up is correct
    swapAxes: false,
  },
